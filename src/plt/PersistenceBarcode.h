...
public:
  ...
  void
  putToAFileHistogramOfBarcodesLengths(char *filename, size_t howMany,
                                       bool shouldWePutAlsoResponsibleBarcodes);
  void
  putToAFileHistogramOfBarcodesLengths(char *filename, size_t beginn,
                                       size_t endd,
                                       bool shouldWePutAlsoResponsibleBarcodes);
  ...
  produceBettiNumbersOnAGridFromMinToMaxRangeWithAStepBeingParameterOfThisFunction(
      unsigned step, double minn = INT_MAX, double maxx = INT_MIN);
  ...
  PersistenceBarcodes
  restrictBarcodesToGivenInterval(std::pair<double, double> interval);

  double computeAverageOfMidpointOfBarcodes();
  double computeAverageOfMidpointOfBarcodesWeightedByLength();

  // this  procedure shift all the barcodes so that the averaged center of
  // barcode is 0 and the averaged length of a barcode is 1.
...
void PersistenceBarcodes::plot(char *filename) {
  ...
  out << "set xrange ["
      << minMaxValues.first - 0.1 * (minMaxValues.second - minMaxValues.first)
      << " : "
      << minMaxValues.second + 0.1 * (minMaxValues.second - minMaxValues.first)
      << " ]" << endl;
  out << "set yrange ["
      << minMaxValues.first - 0.1 * (minMaxValues.second - minMaxValues.first)
      << " : "
  ...
  for (size_t i = 0; i != this->barcodes.size(); ++i) {
    out << this->barcodes[i].first << " " << this->barcodes[i].second << endl;
  }
  out << "EOF" << endl;
  out << minMaxValues.first - 0.1 * (minMaxValues.second - minMaxValues.first)
      << " "
      << minMaxValues.first - 0.1 * (minMaxValues.second - minMaxValues.first)
      << endl;
  out << minMaxValues.second + 0.1 * (minMaxValues.second - minMaxValues.first)
      << " "
      << minMaxValues.second + 0.1 * (minMaxValues.second - minMaxValues.first)
      << endl;
...
bool compareAccordingToLength(const std::pair<double, double> &f,
...
void PersistenceBarcodes::removeBarcodesThatBeginsBeforeGivenNumber(
    double number) {
  ...
  for (size_t i = 0; i != this->barcodes.size(); ++i) {
    if (this->barcodes[i].first > number) {
      ...
      // in the opposite case this->barcodes[i].second <= in which case, we
...
void PersistenceBarcodes::writeBarcodesSortedAccordingToLengthToAFile(
    char *filename) {
  // first sort the bars according to their length
  std::vector<std::pair<double, double>> sortedBars;
  sortedBars.insert(sortedBars.end(), this->barcodes.begin(),
                    this->barcodes.end());
...
void PersistenceBarcodes::putToBins(size_t numberOfBins) {
  ...
  for (size_t i = 0; i != this->barcodes.size(); ++i) {
    ...
    size_t rightBinNumber =
        floor((this->barcodes[i].second - minMax.first) / dx);

    double leftBinEnd = minMax.first + (leftBinNumber + 0.5) * dx;
    double rightBinEnd = minMax.first + (rightBinNumber + 0.5) * dx;

    if (leftBinEnd != rightBinEnd) {
      binnedData.push_back(std::make_pair(leftBinEnd, rightBinEnd));
    }

    if (dbg) {
      cerr << "(" << this->barcodes[i].first << "," << this->barcodes[i].second
           << ") gets mapped to (" << leftBinEnd << "," << rightBinEnd
           << ") \n";
      getchar();
    }
  }
...
void PersistenceBarcodes::setAveragedLengthToOne() {
  ...
  for (size_t i = 0; i != this->barcodes.size(); ++i) {
    double midpoint =
        0.5 * (this->barcodes[i].first + this->barcodes[i].second);
    double length = fabs(this->barcodes[i].first - this->barcodes[i].second) /
                    averageLength;
...
void PersistenceBarcodes::setRange(double beginn, double endd) {
  ...
  for (size_t i = 0; i != this->barcodes.size(); ++i) {
    ...
    double newBegin = beginn + (originalBegin - minMax.first) *
                                   (endd - beginn) /
                                   (minMax.second - minMax.first);
    double newEnd = beginn + (originalEnd - minMax.first) * (endd - beginn) /
                                 (minMax.second - minMax.first);
...
void PersistenceBarcodes::writeToFile(char *filename) {
  ...
  out.open(filename);
  for (size_t i = 0; i != this->barcodes.size(); ++i) {
    out << this->barcodes[i].first << " " << this->barcodes[i].second
        << std::endl;
  }
  out.close();
}
...
double
PersistenceBarcodes::computeAverageOfMidpointOfBarcodesWeightedByLength() {
  ...
  for (size_t i = 0; i != this->barcodes.size(); ++i) {
    double weight = (this->barcodes[i].second - this->barcodes[i].first) /
                    averageBarcodeLength;
    weightedAverageOfBarcodesMidpoints +=
        weight * 0.5 * (this->barcodes[i].second - this->barcodes[i].first);
  }
...
bool compareForHistograms(
    const std::pair<double, std::pair<double, double>> &f,
    const std::pair<double, std::pair<double, double>> &s) {
  return f.first < s.first;
}

void PersistenceBarcodes::putToAFileHistogramOfBarcodesLengths(
    char *filename, size_t howMany, bool shouldWeAlsoPutResponsibleBarcodes) {
  std::vector<std::pair<double, std::pair<double, double>>> barsLenghts(
      this->barcodes.size());
  for (size_t i = 0; i != this->barcodes.size(); ++i) {
    barsLenghts[i] = std::make_pair(
        fabs(this->barcodes[i].second - this->barcodes[i].first),
        std::make_pair(this->barcodes[i].first, this->barcodes[i].second));
  }
  ...
  if (shouldWeAlsoPutResponsibleBarcodes) {
    for (size_t i = 0; i != minn(barsLenghts.size(), howMany); ++i) {
      file << i << " " << barsLenghts[i].first << " "
           << barsLenghts[i].second.first << " " << barsLenghts[i].second.second
           << std::endl;
    }
  } else {
    for (size_t i = 0; i != minn(barsLenghts.size(), howMany); ++i) {
...
void PersistenceBarcodes::putToAFileHistogramOfBarcodesLengths(
    char *filename, size_t beginn, size_t endd,
    bool shouldWeAlsoPutResponsibleBarcodes) {
  if (beginn >= endd) {
    throw("Wrong parameters of putToAFileHistogramOfBarcodesLengths provedure. "
          "Begin points is greater that the end point. Program will now "
          "terminate");
  }
  std::vector<std::pair<double, std::pair<double, double>>> barsLenghts(
      this->barcodes.size());
  ...
  file.open(filename);
  if (shouldWeAlsoPutResponsibleBarcodes) {
    for (size_t i = minn(barsLenghts.size(), beginn);
         i != minn(barsLenghts.size(), endd); ++i) {
      file << i << " " << barsLenghts[i].first << " "
           << barsLenghts[i].second.first << " " << barsLenghts[i].second.second
           << std::endl;
    }
  ...
  std::ofstream file;
  file.open(filename);
  for ( size_t i = minn(barsLenghts.size(),beginn) ; i !=
  minn(barsLenghts.size(),endd) ; ++i )
  {
...
void PersistenceBarcodes::removeShortBarcodes(double minimalDiameterOfBarcode) {
  ...
  for (size_t i = 0; i != this->barcodes.size(); ++i) {
    if (fabs(this->barcodes[i].second - this->barcodes[i].first) >
...
PersistenceBarcodes PersistenceBarcodes::restrictBarcodesToGivenInterval(
    std::pair<double, double> interval) {
  PersistenceBarcodes result;
  result.dimensionOfBarcode = this->dimensionOfBarcode;
  for (size_t i = 0; i != this->barcodes.size(); ++i) {
    if (this->barcodes[i].first >= interval.second)
      continue;
    if (this->barcodes[i].second <= interval.first)
      continue;
    result.barcodes.push_back(
        std::make_pair(std::max(interval.first, this->barcodes[i].first),
                       std::min(interval.second, this->barcodes[i].second)));
  }
  return result;
}
...
std::pair< double , std::vector< std::pair< std::pair<double,double> ,
std::pair<double,double> > > > computeBottleneckDistance( const
PersistenceBarcodes& first, const PersistenceBarcodes& second , unsigned p  )
{
   //If first and second have different sizes, then I want to rename them in the
way that first is the larger one: std::vector< std::pair<double,double> >
firstBar; std::vector< std::pair<double,double> > secondBar;

   firstBar.insert( firstBar.end() , first.barcodes.begin() ,
first.barcodes.end() ); secondBar.insert( secondBar.end() ,
second.barcodes.begin() , second.barcodes.end() );
...
example. Suppose one set of bars consist of two points A, B, and another consist
of a single point C.
   ...
   //
   //Therefore as one can clearly see, the matrix can be parition in to 4
essential parts:
   //    P1 | P2
   //    P3 | P4
   //Where:
...
computeDistanceOfPointsInPlane( firstBar[coll] , secondBar[row] ) << std::endl;
           ...
           {
               //P2
               //distance between point from secondBar and its projection to
diagonal Array[coll][row] = (int)bigNumber*pow(computeDistanceOfPointsInPlane(
...
"\n"; Rcpp::Rcerr << "The distance is : " << Array[coll][row] << std::endl;
           ...
           if ( (coll < firstBar.size()) && (row >= secondBar.size()) )
           {
              //distance between point from firstBar and its projection to
diagonal Array[coll][row] = (int)bigNumber*pow(computeDistanceOfPointsInPlane(
...
std::endl;
   ...
   if (computeBottleneckDistanceDBG)
   {
    ...
    int cost =
hungarian(Array,Result,(firstBar.size()+secondBar.size()),(firstBar.size()+secondBar.size()));
    if (computeBottleneckDistanceDBG)
    {
...
<< firstBar[y - secondBar.size()] << ")  ";}
               ...
               if ( store )
               {
                   ...
                   matching.push_back( std::make_pair(firstBar[x] ,
secondBar[y]) );
               }
           }
      }
    ...
    delete [] Array;
    delete [] Result;
    std::pair< double , std::vector< std::pair<
std::pair<double,double>,std::pair<double,double> > > > result = std::make_pair(
pow(cost/(double)bigNumber,1/(double)p) , matching ); return result;
}
*/

std::vector<unsigned> PersistenceBarcodes::
    produceBettiNumbersOnAGridFromMinToMaxRangeWithAStepBeingParameterOfThisFunction(
        unsigned step, double minn, double maxx) {
  bool dbg = false;
  std::pair<double, double> minMax;
  if (minn == INT_MAX) {
    minMax = this->minMax();
  } else {
    minMax = std::make_pair(minn, maxx);
  }

  std::vector<unsigned> bettiNumbers(step + 1);
  for (size_t j = 0; j != step; ++j) {
    bettiNumbers[j] = 0;
  }

  double dx = (minMax.second - minMax.first) / (double)step;

  for (size_t i = 0; i != this->barcodes.size(); ++i) {
    if (dbg) {
      cerr << "i : " << i
           << " , this->barcodes.size():  " << this->barcodes.size() << endl;
    }
    if (dbg)
      cerr << "For a interval : " << this->barcodes[i] << " we have : \n";
    unsigned first = (this->barcodes[i].first - minMax.first) / dx;
    unsigned second = (this->barcodes[i].second - minMax.first) / dx;
    if (dbg)
