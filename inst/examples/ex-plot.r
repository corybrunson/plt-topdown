...
# palette name
plot(landscape(pd, degree = 0L, min_b = 0, max_b = 1.5, by = 0.01),
     palette = "Cork", lwd = 1)
# palette function name
plot(landscape(pd, degree = 1L, min_b = 0, max_b = 1.5, by = 0.01),
     palette = "topo.colors", lwd = 1, rev = TRUE)
# custom color ramp
plot(landscape(pd, degree = 2L, min_b = 0, max_b = 1.5, by = 0.01),
