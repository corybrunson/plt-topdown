...
integrate_power <- function(x, p) {
  ...
  if (any(diff(x[, 1L]) <= 0)) stop("Abscissa  coordinates are not increasing.")
...
pl1 <- landscape(pd1, degree = 1, max_x = 1.5, dx = 0.01)
pl2 <- landscape(pd2, degree = 1, max_x = 1.5, dx = 0.01)
