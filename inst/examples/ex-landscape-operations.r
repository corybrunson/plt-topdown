...
# create two landscapes from the same sampling procedure
x <- tdaunif::sample_torus_tube(4, 2.5)
...
x_pl <- landscape(x_pd$pairs[[1]], exact = FALSE, max_x = 2.5, dx = 0.1)
y_pl <- landscape(y_pd$pairs[[1]], exact = FALSE, max_x = 2.5, dx = 0.1)
...
pl <- landscape(pd, degree = 1, exact = FALSE, dx = 0.1, max_x = 2)
...
for (i in seq(100)) {
  ...
  pli <- landscape(pdi, degree = 1, exact = FALSE, dx = 0.1, max_x = 2)
